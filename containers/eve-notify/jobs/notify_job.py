...
class NotifyJob():
    ...
    def notify(self, user_segment, tip):
    ...
    def run(self):
        ...
        if trying_tip:
            status_code = self.notify("Trying Conceive Users", trying_tip['description'])
            if status_code == 200:
                self.update_tip(trying_tip['_id']['$oid'])
                get_module_logger().info('Notificação recebida pelos usuários tentantes...')
        if pregnant_tip:
            status_code = self.notify("Pregnant Users",pregnant_tip['description'])
            if status_code == 200:
